...
const frequencyOfUpdate = 4 * 1 * 1000; //(1000 = 1 second. send to animation every 4 min)
const broadcastFrequency = 5; // (save in history every 5 cycles (minimizing size))
...
function nextPopulation(h,i) { // h=habitat, i=species
    ...
    if ((modelIndex = isResource(h,i)) >= 0) {
        ...
        var sum2 = 0;
        for (var j=0; j<model['resourceIndex'][h].length; j++) {
if (DEBUGflag && h==3 && i==5) console.log('index: ' + model["resourceIndex"][h][j]);        
if (DEBUGflag && h==3 && i==5) console.log("second term: " + state['populations'][h][model['resourceIndex'][h][j]]);        
if (DEBUGflag && h==3 && i==5) console.log('first term: ' + model['alpha'][h][modelIndex][j]);        
if (DEBUGflag && h==3 && i==5) console.log('inner sum1: ' + sum1);

            sum1 += (model['alpha'][h][modelIndex][j] * state['populations'][h][model['resourceIndex'][h][j]]);}
if (DEBUGflag && h==3 && i==5) console.log('outer sum1: ' + sum1);
        for (var k=0; k<model['herbivoreIndex'][h].length; k++)
            sum2 += ((model['a'][h][modelIndex][k] * state['populations'][h][model['herbivoreIndex'][h][k]]) / 
                (1 + model['q'][h][k] * state['populations'][h][model['herbivoreIndex'][h][k]]));
if (DEBUGflag && h==3 && i==5) console.log('sum2: ' + sum2);
        var exponent = model['r'][h][modelIndex] * 
            (model['K'][h][modelIndex] - sum1)/model['K'][h][modelIndex] - sum2;
if (DEBUGflag && h==3 && i==5) console.log('exponent: ' + exponent); //throw new Error("Something went badly wrong!");}
        var speed = 1;
