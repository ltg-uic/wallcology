function DialogBox(n, cH, cW, c, colour){
	//( namesArr, oldHeight, oldWidth, ctx, species, num, direction, buttonColour, headingText, promptText );
	...
	*/
	this.context = c;
	//name needs to be the same as line name
	this.name = n;
	this.height = 200;//n.length * (graphHeight+paddingY) + yOffset + marginTop+ marginBottom + btnHeight + paddingY;
	...
	//var btnX; 
	//var btnY; 
	var btnX = canvasWidth - btnWidth - padding;
	var btnY = canvasHeight - this.height + paddingY; //btnHeight - paddingY - marginBottom;
	...
	*/
	...
	this.button = new GenericButton("[ Close ]", btnX, btnY, btnHeight, btnWidth, c, this.colour, "#FFFFFF", "24pt 'Roboto'", 8);
	...
	*/
	...
	function setupVars(p, x, y, closeBtn){
		//p = phase "init"
	...
	this.setCanvasWidthHeight = function(cw, ch){
		...
		setupVars("orientation", this.x, this.y, this.button);
	}
	this.onMouseUp = function (mouseX,mouseY) {
		if( hitTest(mouseX, mouseY, this.button) ){
			...
			*/
			...
			this.dispatch(this.EVENT_CLOSE);
		}
	}
	/*
...
DialogBox.prototype = new EventDispatcher();
